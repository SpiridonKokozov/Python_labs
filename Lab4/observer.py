...
class StorePublisher(Publisher):
    ...
    def notify(self):
        print("Publisher:", 'red'), "Оповещаю подписчиков..."
        subscribers_reacts = []
    ...
    def goods_arrival(self, goods):
        ...
        print("Publisher:", 'red'), f"Поступил новый товар - {self.new_goods}"
        self.notify()
...
class HoodiesSubscriber(Subscriber):
    ...
    def update(self, publisher):
        if publisher.new_goods == "толстовка":
            react = "SneakersSubscriber:" + f"{self.name} реагирует на новое поступление толстовки"
            return react
        else:
            return 1
